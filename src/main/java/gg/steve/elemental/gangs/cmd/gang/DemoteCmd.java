...
public class DemoteCmd {
    ...
    public static void demote(CommandSender sender, String[] args) {
        ...
        if (!player.hasGangPermission(PermissionNode.DEMOTE)) {
            MessageType.INSUFFICIENT_GANG_PERMISSION.message(player, PermissionNode.DEMOTE.get());
            return;
        }
        ...
        player.getGang().demote(target.getPlayerId());
        MessageType.DEMOTE.gangMessage(player.getGang(), target.getName(), target.getRole().name(), player.getName());
    }
}
