...
public class KickCmd {
    ...
    public static void kick(CommandSender sender, String[] args) {
        ...
        if (!player.hasGangPermission(PermissionNode.KICK)) {
            MessageType.INSUFFICIENT_GANG_PERMISSION.message(player, PermissionNode.KICK.get());
            return;
        }
        ...
        if (!target.isOnline()) {
            GangManager.getRequestManager().addKickRequest(player.getGang().getGangId(), target.getPlayerId());
        }
        MessageType.KICK.gangMessage(player.getGang(), target.getName(), player.getGang().getName(), player.getName());
        player.getGang().removePlayer(target.getPlayerId());
        GangPlayerManager.updateGangPlayer(target.getPlayerId());
    }
}
