package gg.steve.elemental.gangs.gui;
...
public abstract class AbstractGui {
    ...
    public AbstractGui(YamlConfiguration config, String type, Integer... size) {
        ...
        if (!type.equalsIgnoreCase("none")) {
            this.inventory = Bukkit.createInventory(null, InventoryType.valueOf(type.toUpperCase()), ColorUtil.colorize(config.getString("name")));
        } else {
    ...
    public AbstractGui(YamlConfiguration config, String type, Role role, Integer... size) {
        ...
        if (!type.equalsIgnoreCase("none")) {
            ...
            this.inventory = Bukkit.createInventory(null, size[0], ColorUtil.colorize(config.getString("name").replace("{role}", role.toString())));
        }
        this.clickActions = new HashMap<>();
        inventoriesByID.put(getInventoryID(), this);
    }

    public AbstractGui(YamlConfiguration config, String type, Gang gang, Integer... size) {
        this.inventoryID = UUID.randomUUID();
        if (!type.equalsIgnoreCase("none")) {
            this.inventory = Bukkit.createInventory(null, InventoryType.valueOf(type.toUpperCase()), ColorUtil.colorize(config.getString("name").replace("{gang}", gang.getName())));
        } else {
            this.inventory = Bukkit.createInventory(null, size[0], ColorUtil.colorize(config.getString("name").replace("{gang}", gang.getName())));
        }
        this.clickActions = new HashMap<>();
        inventoriesByID.put(getInventoryID(), this);
    }
